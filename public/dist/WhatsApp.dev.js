...
var inpotd = document.querySelector("#inpotd");
var intext = document.querySelector("#intext");
var marginvov = document.querySelector(".marginvov");
var sd = document.querySelector("#sd");
var red = document.querySelector(".red");
var text_tel = document.querySelector(".text_tel");
var texttext = document.querySelector(".texttext");
var intel = document.querySelector(".intel");
var html = ""; // הגדרות ראשוניות על הינפוט של הטלפון

inputext.focus();
text_tel.className = "ohverinpot"; // הוספת - אחרי 054 אוטומטי

inputext.addEventListener("keyup", function () {
  var Mobile_Phone = inputext.value;
...
function next() {
  ...
  if (Mobile_Phone.length < 1) {
    ...
    window.open("https://wa.me/972".concat(Mobile_Phone.replace('-', ''), "?text=").concat(encodeURIComponent(inpotd.value)));
  }
...
function copyy(event) {
  ...
  if (Mobile_Phone.length < 1) {
    ...
    if (inpotd.value.length > 0) {
      intext.value = "https://wa.me/972".concat(inputext.value.replace('-', ''), "?text=").concat(encodeURIComponent(inpotd.value));
      document.execCommand('copy');
    } else {
      intext.value = "https://wa.me/972".concat(inputext.value.replace('-', ''));
    }
